...
/** T4 综合扫描 v2：当前 HMB 结构（baseNoise+inland+maskGate），扫 baseGain × massifGain × gateLo，
 *  用 getHeight(侵蚀后) 测全 13 契约。找全绿组合。 */
public class ProbeTune20 {
    static long SEED = 20260803L;
    static final int GRID = 256;
    static final int[][] M6_WINDOWS = {{0, 0}, {-1024, 0}, {512, 512}};

    public static void main(String[] args) {
        float[] baseGains = {30f, 60f, 90f, 120f};
        float[] massifs = {600f, 800f, 1000f, 1200f};
        float[] gateLos = {0.55f, 0.65f, 0.75f};
        int best = 0; String bestLine = "";
        int total = baseGains.length * massifs.length * gateLos.length; int done = 0;
        for (float bg : baseGains) for (float mg : massifs) for (float gl : gateLos) {
            ...
            if (pass >= 9) System.out.println(r + " (bg=" + (int)bg + " massif=" + (int)mg + " gateLo=" + gl + ")");
        }
